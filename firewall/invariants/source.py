...
The structural invariants are properties of the *code*, not of any
...
def parse_module(path: Path) -> ast.Module:
    ...
    twenty-six invariants parses the whole package independently, and
    each census walks the trees it parsed, so a single ``assert_all``
    parsed every module twenty-six times. Nothing cached it: the suite
