...
attestation. A fresh checkout has none, so a source-only run
...
-- and runs all twenty-two against it, so ``--exercise --strict`` is a
gate that
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Check the security invariants (v2.2-v3.1) against "
