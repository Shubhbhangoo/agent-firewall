...
lifecycle, a recorded side-effect verification, a recorded external
attestation, and a sampled clock. A fresh checkout has none, so a
source-only run
...
-- and runs all twenty-three against it, so ``--exercise --strict`` is a
gate that
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Check the security invariants (v2.2-v3.2) against "
